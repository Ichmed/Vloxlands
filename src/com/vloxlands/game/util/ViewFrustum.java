...
public class ViewFrustum
{
	...
	public void calculateViewFrustum(Vector3f camPos, Vector3f camRot, Vector3f up, float nearDist, float farDist)
	{
		...
		ntl = Vector3f.sub(Vector3f.add(fc, (Vector3f) up.scale(Hnear / 2), null), (Vector3f) right.scale(Wnear / 2), null);
		ntr = Vector3f.add(Vector3f.add(fc, (Vector3f) up.scale(Hnear / 2), null), (Vector3f) right.scale(Wnear / 2), null);
		nbl = Vector3f.sub(Vector3f.sub(fc, (Vector3f) up.scale(Hnear / 2), null), (Vector3f) right.scale(Wnear / 2), null);
		nbr = Vector3f.add(Vector3f.sub(fc, (Vector3f) up.scale(Hnear / 2), null), (Vector3f) right.scale(Wnear / 2), null);
	...
	{
		for(Plane p : planes) if(Vector3f.sub(p.getStartingPoint(), point, null).length() < 0) return false;
		return true;
	}
}
